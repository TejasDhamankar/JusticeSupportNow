...
export default function CasesPage() {
  ...
  return (
    <main className="flex min-h-screen flex-col" style={{ backgroundColor: colors.background, color: colors.textPrimary, '--accent-green': colors.accentGreen } as React.CSSProperties}>
      <Header />

      {/* Hero Section */}
      <section className="relative pt-32 pb-20 overflow-hidden" style={{ backgroundColor: colors.background }}>
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="max-w-4xl ml-0 text-left">
            ...
            <h1 className="text-5xl md:text-6xl font-serif font-bold mb-6 tracking-tighter" style={{ color: colors.textPrimary }}>
              Standing for Justice
              <br />
              <span style={{ color: colors.textSecondary }}>Empowering Individuals</span>
            </h1>

            <div className="w-24 h-1 mb-6 rounded-full" style={{ background: colors.accentGreen }} />

            <p className="text-lg sm:text-xl mb-8 leading-relaxed max-w-3xl" style={{ color: colors.textSecondary }}>
            ...
            <div className="flex flex-col sm:flex-row items-start gap-4">
               <Button asChild size="lg" className="group relative font-bold py-4 px-8 rounded-lg shadow-lg overflow-hidden transition-all duration-300 text-lg" style={{ backgroundColor: colors.accentGreen, color: colors.background }}>
              ...
              <Button asChild variant="outline" size="lg" className="rounded-lg py-4 px-8 font-semibold transition-colors hover:border-[var(--accent-green)] hover:text-[var(--accent-green)] text-lg" style={{ borderColor: colors.border, color: colors.textPrimary, backgroundColor: 'transparent' }}>
        ...
        <section id="hot-cases" className="py-20" style={{ backgroundColor: colors.cardBackground }}>
          <div className="container mx-auto px-4 relative z-10">
            <div className="text-center mb-16">
              ...
              <h2 className="text-4xl md:text-5xl font-serif font-bold text-white mb-6 tracking-tighter" style={{ color: colors.textPrimary }}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-12">
              {hotCases.map((caseType) => (
                <Card key={caseType.id} style={{ backgroundColor: colors.darkBlue, borderColor: colors.borderGray }} className="group overflow-hidden shadow-2xl rounded-xl border h-full flex flex-col relative before:absolute before:top-0 before:left-0 before:w-full before:h-[1px] before:bg-gradient-to-r from-transparent via-[var(--accent-green)] to-transparent transition-all duration-300 hover:border-[var(--accent-green)] hover:shadow-[0_0_20px_rgba(42,170,138,0.3)]">
                  <div className="absolute top-4 right-4 z-10" style={{ backgroundColor: colors.background, borderColor: colors.border }}>
                  ...
                  <CardContent className="p-6 flex-grow flex flex-col">
                    <div className="flex items-center justify-between mb-3">
                      <Badge variant="outline" className="font-semibold text-xs border-transparent" style={{ backgroundColor: colors.border, color: colors.textSecondary }}>{caseType.slug}</Badge>
                    ...
                    <h3 className="text-xl font-bold mb-3 group-hover:text-[var(--accent-green)] transition-colors" style={{ color: colors.textPrimary }}>{caseType.title}</h3>
                    <p className="mb-6 flex-grow text-base leading-relaxed" style={{ color: colors.textSecondary }}>{caseType.shortDescription}</p>

                    <div className="rounded-lg p-3 mb-6" style={{backgroundColor: colors.border + '80'}}>
                        <div className="flex items-center text-sm font-bold mb-2" style={{color: colors.textPrimary}}>
                            <DollarSign className="w-4 h-4 mr-1.5" style={{color: colors.accentGreen}} /> Possible Settlement
                        </div>
                        <p className="text-sm" style={{color: colors.textSecondary}}>Estimated settlements starting at $15,000, depending on your case</p>
                    </div>
                    ...
                    <div className="mt-auto space-y-3">
                      ...
                      <Button asChild variant="outline" className="w-full rounded-lg font-semibold transition-colors hover:border-[var(--accent-green)] hover:text-[var(--accent-green)]" style={{ borderColor: colors.border, color: colors.textSecondary, backgroundColor: 'transparent' }}>
      ...
      <section className="py-20" style={{backgroundColor: colors.background}}>
          <div className="container mx-auto px-4 relative z-10">
              <div className="mb-16 max-w-4xl ml-0 text-left">
                  ...
                  <p className="text-lg sm:text-xl leading-relaxed max-w-3xl" style={{ color: colors.textSecondary }}>
                      Discover every open case handled through Justice Support Now. <span className="font-bold" style={{ color: colors.textPrimary }}> Each claim is a step toward accountability and fair compensation.</span>
                  </p>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                  {caseTypes.map((caseType) => (
                      <Card key={caseType.id} style={{ backgroundColor: colors.cardBackground, borderColor: colors.border }} className="group overflow-hidden shadow-2xl rounded-xl border h-full flex flex-col relative before:absolute before:top-0 before:left-0 before:w-full before:h-[1px] before:bg-gradient-to-r from-transparent via-[var(--accent-green)] to-transparent transition-all duration-300 hover:border-[var(--accent-green)] hover:shadow-[0_0_20px_rgba(42,170,138,0.3)]">
                          <div className="relative h-56 w-full overflow-hidden">
                              <Image src={caseType.imageUrl} alt={caseType.title} fill className="object-cover group-hover:scale-105 transition-transform duration-500"/>
                              <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent" />
                              <div className="absolute top-4 left-4">
                                  <Badge className="font-bold text-xs" style={{ backgroundColor: colors.background, color: colors.textSecondary }}>{caseType.slug}</Badge>
                              </div>
                              {caseType.featured && (
                          ...
                          <CardContent className="p-6 flex-grow flex flex-col">
                              ...
                              <div className="mt-auto space-y-3">
                                  ...
                                  <Button asChild variant="outline" className="w-full rounded-lg font-semibold transition-colors hover:border-[var(--accent-green)] hover:text-[var(--accent-green)]" style={{ borderColor: colors.border, color: colors.textPrimary, backgroundColor: 'transparent' }}>
                                      <Link href={`/cases/${caseType.slug}`}>Case Information</Link>
                                  </Button>
                              </div>
                          </CardContent>
      ...
      <section id="case-evaluation" className="py-20" style={{backgroundColor: colors.cardBackground}}>
        <div className="container mx-auto px-4 relative z-10">
            <div className="max-w-4xl ml-0 text-left">
                ...
                <h2 className="text-4xl md:text-5xl font-serif font-bold mb-6 tracking-tighter" style={{ color: colors.textPrimary }}>
                    Not Sure If You Qualify?
                    <br />
                    <span style={{ color: colors.textSecondary }}>We’re Here to Help You Find Out.</span>
                </h2>
                <div className="w-24 h-1 my-6 rounded-full" style={{ backgroundColor: colors.accentGreen }} />
                <p className="text-lg sm:text-xl mb-8 leading-relaxed max-w-3xl" style={{ color: colors.textSecondary }}>
                ...
                <div className="flex flex-col sm:flex-row items-start gap-4">
                    <Button asChild size="lg" className="group relative font-bold py-4 px-8 rounded-lg shadow-lg overflow-hidden text-lg" style={{ backgroundColor: colors.accentGreen, color: colors.background }}>
