            ...
            <div className="container mx-auto px-4 relative z-10">
                ...
                >
                    ...
                    <motion.div variants={itemVariants} className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-8 my-16">
                        {keyFacts.map((fact) => (
                            <div key={fact.label} className="text-center">
                                ...
                                <p className="font-bold text-xl md:text-2xl" style={{ color: colors.textPrimary }}>{fact.value}</p>
                                <p className="text-xs md:text-sm" style={{ color: colors.textSecondary }}>{fact.label}</p>
                            </div>
                        ))}
                    </motion.div>
                    ...
                    <motion.div variants={itemVariants} className="relative mt-16">
                        ...
                        {/* The vertical line for larger screens */}
                        <div className="absolute left-1/2 top-0 h-full w-0.5 hidden lg:block" style={{ background: colors.border }} />
                        ...
                        <div className="relative flex flex-col gap-12">
                            {legalPathway.map((step, index) => (
                                <div key={index} className="flex items-center justify-center">
                                    ...
                                    {/* Timeline Icon */}
                                    <div className="relative flex-shrink-0 hidden lg:block" style={{ order: index % 2 === 0 ? 2 : 1 }}>
                                        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 p-4 rounded-full" style={{ backgroundColor: colors.border}}>
                     ...
                     {/* Final CTA */}
                    <motion.div variants={itemVariants} className="mt-16 pt-12 border-t" style={{ borderColor: colors.border }}>
